...
def plot_stock_data(stock_data):
    fig = px.line(stock_data, x=stock_data.index, y='Close', title=print_word('Stock Prices Over the Last Year'))
...
Linear regression is widely used in various fields for tasks such as predicting stock prices, housing prices, sales forecasting, and many other applications where understanding the relationship between variables is crucial.  
    ...
    st.write(print_word(f"Y = {float(m)}x + {float(y.iloc[0])}"))

st.set_page_config(
    page_title=print_word("Stocks analyzer"),
...
def investment(stock_symbol, stock_data):
    ...
    if stock_data is not None:
        ...
        st.success(
            print_word(f"You would have approximately {potential_returns:.2f}$ based on the percentage change of {percent_change:.2f}%."))
...
def homepage():
    ...
    page = st.sidebar.radio(print_word("Navigation"), ["Sign Up", "Sign In"])
