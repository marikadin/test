...
def language_chooser():
    if 'chosen_language' not in st.session:
        st.session['chosen_language'] = 'en'

    st.header(print_word("Choose a language"))
    language_options = ['Russian', 'English', 'Hebrew']
    st.session['chosen_language'] = st.selectbox("Choose a language", language_options)
    st.session['chosen_language'] = st.session['chosen_language'][:2].lower()
...
page = st.sidebar.radio(print_word("Select Page"), [print_word("Home"), print_word("Stock Analysis"),print_word("language chooser")])
